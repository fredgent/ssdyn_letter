...
\fg{$\eta<\eta_{\rm crit}$}, the SSD saturates at about 5\% of kinetic energy
...
\fg{We now examine whether the field growth seen in our models could
  be due to tangling.
 ...
 \fg{Instead, the} forcing \fg{is} distributed at scales \fg{of roughly 
 60--200~pc
   \citep{joung2006,avillez2007,HSSFG17}, or $k \sim 30$--105~kpc$^{-1}$}.

 \fg{In \fg{Figures~\ref{fig:eb-res} and~\ref{fig:eb-nu}} we indeed
 demonstrate strong exponential growth over multiple orders of magnitude
 \fg{for sufficiently low resistivity, at varying supernova} rate $\dot\sigma$,
 numerical resolution $\dx$, and physical viscosity $\nu$. 
 \fg{Apparently l}inear growth \fg{occurs only} with high physical resistivity.}

 \fg{We now turn to the power spectra. Figure\,\ref{fig:4power} shows
 compensated spectra over time during \fg{intervals} that}
 \fg{span epochs with distinct rates of SSD  growth followed by saturation.}
