...
(and by implication magnetic Reynolds number, Rm) 
to identify characteristics of the small-scale dynamo (SSD) under
...
%FAG: we see from Fig 5 panel (h) that the flow is not significantly
%FAG: altered by higher SN, but the growth rate of B appears higher
%FAG: due to the higher temperature
%nature of the flow.
%FAG: added
...
%FAG: exclusion -> suppression
%MJK Exclusion I do not understand... Even 2pc and 4pc system grow mgf. Why do we think this is not SSD?
sub-parsec scale and a trend towards the suppression of SSD for resolution
...
Across the modelled range of 0.5 to 4 parsec resolution we find with sufficiently low 
resistivity the SSD 
...
will grow exponentially if the system supports the large-scale dynamo instability}
%NS.
...
is characterised by exponential growth \fag{Nishant, please add some relevant citation to this expected behaviour. MJK: I do not think we need a ref. to the expectation that the dynamo is growing exponentially, but perhaps to the expected powerlaw, where the old Kazantsev paper 1968 should be cited.}, growing over 10 orders of magnitude in 
...
at the grid scale to resolve small-scale instabilities \citep[see, e.g.,][]{ABGS20}.
