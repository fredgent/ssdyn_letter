...
% mm [rearranged sentence now that I've added another clause]
    inconsistent with simple tangling of magnetic fields, but
consistent with SSD action,
% mm [added]
      reproducing and confirming models by Balsara et al. that did not
      include physical resistivity $\eta$.}
We vary
% mm the physical resistivity,
...
% mm prior to onset of a dynamo. Instead,
 For models reproducing \BKM\ this seed is substituted by
 a uniform $10$~nG background field.}
 %MJK If we do have the space, I would add here 'following \BKM.' Otherwise it is a bit unclear why have we chosen such a value.
...
{Figure~\ref{fig:eb-res} shows
 ...
 $\dot\sigma = 8\SNr$ than the low value of $0.5\SNr$ that we use in our
...
\fg{
  % mm We show in Figure~\ref{fig:eb-res}(d) how there is greater
  % kinetic energy with $\dot\sigma=8\SNr$ than $0.2\SNr$, and that
  % the higher SN rate induces
 ...
 %\fg{Figure\,\ref{fig:eb-res}\,(d) shows mean sonic Mach number initially higher
 ...
 %\citep{Haugen:2004M}.
 %\fg{However, consistent with the earlier study, SSD increases for $8\SNr$
