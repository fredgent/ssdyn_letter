...
%-------------------------------------------------------------------------------
 ...
 {a physical resistivity of $\eta=10^{-5}$ \fg{(panels a1, a2)}
 makes no impact on field
   ...
   comparison of the $\eta=10^{-4}$ and \fg{$10^{-3}$ cases (panels c1 to d2)}.
 }

 {When we consider $e_B$ for the models {with} only numerical
 viscosity
 %(Fig\,\ref{fig:eb-nu} a, c, d), $\eta\geq10^{-3}$ initially appears
 %NS: I suppose we mean only a1, as a2 has finite nu; pls check
 %FAG: a2 include 4 mods without nu
 (Fig\,\ref{fig:eb-nu} a, c, d), $\eta\geq10^{-3}$ initially appears
 %NS: except in d2, all panels have eta <= 10^-3
 %NS: should we then add something like "not shown" then? a1 and c2, it may not be obvious
 %NS: to the reader. But certainly no need to add more curves, but just saying explicitly
 %NS: might help
 %FAG: all include eta=1e-3 initially even at high resolution these decay. 
 %FAG: only b1 and d2 have runs with eta>1e-3 and these definitely decay.
 %FAG: of course when Pm geq 1 it becomes easier to excite a dynamo.
 %FAG: we could increase viscosity to 1e-2 and find whether the dynamo can be
 %FAG: excited for eta=5e-3? Given these high values are so much further away
 %FAG: from the ISM parameters, perhaps it is safe to conclude the SSD is 
 %FAG: ubiquitous in the ISM?
 ...
 This bottleneck shifts to lower $k$ as $\dx$ \fg{increases}
 {(panels a--c) \fg{or $\dot\sigma$ decreases} (panel d).}
...
\begin{figure}
  ...
  \begin{picture}(0,0)(0,0)
    \put(125,483){\sf{\fg{$\delta x=0.5 $}}}
    \put(125,468){\sf{\fg{$\dot\sigma=\frac{1}{5}\SNr$}}}
    \put(125,455){\sf{\fg{$\eta=10^{-4}$}}}
    \put(125,366){\sf{\fg{$\delta x=0.5 $}}}
    \put(125,353){\sf{\fg{$\dot\sigma=\frac{1}{5}\SNr$}}}
    \put(125,340){\sf{\fg{$\eta=10^{-4}$}}}
    \put(125,175){\sf{\fg{\BKM, $\eta=0$}}}
    \put(125, 50){\sf{\fg{\BKM, $\eta=0$}}}
    \put(125,201){\sf{\fg{$\delta x=0.78$}}}
    \put(125, 76){\sf{\fg{$\delta x=0.78$}}}
    \put(125,188){\sf{\fg{$\dot\sigma=          8\SNr$}}}
    \put(125, 63){\sf{\fg{$\dot\sigma=          8\SNr$}}}
...
and kinetic (a2, b2) energy {at times given in the legends
  {(combined for each pair)} for $\dx$\fg{, $\dot\sigma$ and $\eta$ indicated.
Viscosity is $\nu=0$}.}
 \label{fig:4power}}
\end{figure}
%-------------------------------------------------------------------------------
...
{We now examine whether the field growth seen in our models could
 ...
 this signature disappears and the peak shifts to high $k$\fg{, suggesting a healthy SSD}.}
 ...
 dominated by SSD. {O}ur other models \fg{with only a weak seed field} 
  confirm \fg{that} an imposed field is not required.
  %NS: a note: when we say that an imposed field is not required, it could
  %NS: give an impression that no seed field is needed whatsoever.
  %NS: instead, what we perhaps mean is that no uniform imposed field is not needed,
  %NS: if weak magnetic noise is present.
  %FAG: added a qualifier 
 }
