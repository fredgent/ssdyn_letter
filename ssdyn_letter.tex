 ...
 growth at equivalent resolution.}
{
 ...
 %{Figure\,\ref{fig:eb-res}\,(d) shows mean sonic Mach number initially higher
 %for $\dot\sigma=8\SNr$ than $0.2\SNr$, but a higher dynamo growth rate.}
 %Higher $\Ms$ has been demonstrated to impede an SSD in isothermal turbulence
 %\citep{Haugen:2004M}.
 %{However, consistent with the earlier study, SSD increases for $8\SNr$
 %after 10\,Myr and 40\,Myr $0.2\SNr$, as $\Ms$ reduces below 1 and hot
 %fractional volume increases.}
...
%-------------------------------------------------------------------------------
 ...
 {a physical resistivity of $\eta=10^{-5}$ makes no impact on field
   ...
   %NS: which panels we mean here?
   %NS: c1 and c2 may be appropriate as delta x is same and only dot{sigma}
   %NS: is different, but then no 10^-5 case in c1.
   %NS: One possibility could be to just highlight 10^-3 and 10^-4 in
   %NS: the last sentence above, or something else perhaps is meant?
 }
 ...
 (Fig\,\ref{fig:eb-nu} a1, c, d), $\eta\geq10^{-3}$ initially appears
 ...
 %NS: to me, fig 3 suggests that bottleneck shifts to lower k when delta x increases,
 %NS: agreeing with above, but it shifts to higher k when dot{sigma} increases, unlike
 %NS: what is meant in the sentence above. Please check.
 ...
 efficient the SSD.
 %NS: yes, and that's why I believe we have 10^-3 dynamo in d2 but not in d1,
 %NS: as more scales are available at larger \dot{sigma}; see comments just above
...
{We now examine whether the field growth seen in our models could
 ...
 %NS: b1 seems the Balsara model with imposed field. Should we indicate this
 %NS: in Fig5 caption to meke it explicit and without increasing the word count?
 %the imposed field
 %NS: fullstop
 the imposed field.
 ...
 %NS: added a bit in the end; pls check
