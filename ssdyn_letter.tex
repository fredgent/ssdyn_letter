...
{Magnetic field growth occurring in our models appears consistent
  with SSD action, and inconsistent with simple tangling of magnetic fields.}
We vary the physical resistivity, {$\eta$}, as well as the numerical
...
Across the modelled range of 0.5 to 4~{pc} resolution we find that for
...
%==============================================================================
 ...
 % mm [space] This {L}etter addresses the necessary conditions and properties
 % of a 
     \fg{We here study the}
...
% mm Applying a weak imposed uniform field with ideal magnetohydrodynamics (MHD),
% dependencies on magnetic Reynolds (Rm) or Prandtl (Pm) numbers were not
% explored.}
%MJK I would remove "relatively strong", as it is not really strong in comparison to our initial condition.
%      They further imposed a relatively strong initial field and
%FAG: not only initial
...
%==============================================================================
 ...
 % mm [space, relevance] in kinetic energy {spectra for both models} and
...
% mm [rearrange to reduce confusion]
 ...
 \fg{We apply a random initial field, 1~nG at the onset of dynamo,
 which excludes tangling of an imposed field as a source of magnetic
 amplification.
 Except in models reproducing \BKM\ we instead impose only a uniform
 $10$~nG background field.}
 %We apply a random 1~nG initial field, small enough {to} exclude
 %tangling of {the} imposed field {as} a source of magnetic
 %amplification, except {in models reproducing \BKM, where we impose a uniform $10$~nG 
 %background field.} 
...
\caption{
 ...
 %MJK Energy units better kept same as in (c)
 %FAG have revised (d) 
 %FAG if we do not normalise by e_K, then the convergent result for the 
 %FAG saturation at ~5% is not so apparent as the low resolution runs have
 %FAG lower e_K for equivalent parameters.
 %FAG So far we use consistent y-axes for Fig 2 a and b and Fig 4.
 %FAG If we change, do we do so for all or only this Figure
...
{
  % mm [space] For comparison to our models with physical diffusivity and
  % understanding of
        \fg{To understand} 
...
{Figure~\ref{fig:eb-res} shows
 % mm magnetic energy as a function of time for  models with $\eta =
 % 10^{-4}$ and $10^{-3}$, as well as a direct comparison of our
 % technique to \BKM. We find
 ...
 % mm ; we} include a lower resolution run {that} demonstrate{s}
 % consistency with \BKM.}
 
 \fg{Figure\,\ref{fig:eb-res}\,(d) shows the mean sonic Mach number
 %MJK not always larger than 1
 %FAG changed to gtrsim
 ...
 \citep{Haugen:2004M}.
 %MJK Also this figure now seems to imply that Ms>1 impedes
 %MJK SSD, as only when it drops below 1, SSD bursts are seen.
 %MJK Maybe this is just an unlucky co-incidence, but the
 %MJK contradiction we claim is not really visible.
 %FAG in 8SN Ms is above 1 to about 10 Myr, falling below 1 appears
 %FAG to correspond to the saturation stage or late kinematic stage for 1.56pc
 %FAG the Ms trend below 1 for 0.2SN appears also after 40Myr corresponding
 %FAG to post-kinematic stage for both 0.5 and 1pc resolution.
 %FAG Ms is higher in the kinematic phase for 8SN than 0.2SN 
 ...
 We can also examine the
   % mm [space] properties of the flow and magnetic field through
   % their
...
%-------------------------------------------------------------------------------
 ...
 %FAG: moved up where Figure 2 discussed.
