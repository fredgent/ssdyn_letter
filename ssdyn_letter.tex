...
\fg{Magnetic field growth occurring in our models appears consistent
...
the critical resistivity \fg{below} which an SSD occurs is
...
Mach numbers.
SSD\fg{s} \fg{in the supernova-driven interstellar medium} commonly exhibit erratic growth.}
...
%==============================================================================
 ...
 growth at correspondingly short \fg{timescales}.
...
\citet[][with additional analysis by \citealt{EGSFB16}]{Gent:2013b}
 ...
 Previous experiments (\fg{\citealp[e.g.,][hereafter \BKM]{BKMM04};
...
%==============================================================================
 ...
 \fg{Here we specifically seek t}o illustrate
...
\section{\fg{Supernova-driven} turbulence model design} \label{sec:model}
 ...
 \fg{In models aiming to reproduce \BKM, we impose a uniform $10$~nG 
 background field.}
 \fg{Otherwise we apply only a} random 1~nG seed field \fg{to} exclude
 tangling of \fg{the} imposed field \fg{as} a source of magnetic amplification.
...
%<<<<<<< 5e9c75fac65a177a63184cf3b0000c18af08c7bb
...
%=======
  %be due to field tangling.
  be due to tangling.
  In Sect.~\ref{sec:ssd-tang} we argued
    %FAG based on our isothermal models
    that tangling should produce linear growth 
    %FAG rate and that only an inverse
    %Kazantsev cascade should occur, with dissipation dominating scales
    rate, with dissipation dominating scales
  below the forcing range. Conversely, an SSD leads to exponential
  growth and a
    %FAG direct
    Kazantsev cascade extending below the forcing scale.}

 SN-driven turbulence does not have a \fg{single} forcing scale, 
   %FAG unlike the
   %simplified model in Section\,\ref{sec:ssd-tang},
 %  because of the
 %heterogeneous ISM structure and random explosion locations.
   because of \fg{explosions randomly located in} the heterogeneous ISM.
 \fg{Instead, the} forcing 
   %FAG scale
   \fg{is} distributed at scales
%>>>>>>> Merging Mordecai
 ...
 % rather than description.  Please check to see if it is correct]
  %FAG Do you mean 2 and 4?
  %\mm{In Figures~\ref{fig:3power} and~\ref{fig:4power} we indeed demonstrate
\mm{In \fg{Figures~\ref{fig:eb-res} and~\ref{fig:eb-nu}} we indeed demonstrate
strong exponential growth over multiple orders of magnitude
  %FAG
  %so long as
  %the resistivity remains low enough, despite variation in supernova
  \fg{for sufficiently low resistivity, at varying supernova}
rate $\dot\sigma$, numerical resolution $\dx$, and physical viscosity
$\nu$. 
  \fg{Apparently l}inear growth 
  %FAG
  %only appears to occur
  \fg{occurs only}
  % in models
  with high
...
\mm{We now turn to the power spectra. Figure\,\ref{fig:4power} shows
  compensated spectra over time during 
  %FAG a period
  \fg{intervals} that}
...
\fg{
  ...
  span epochs with distinct rates of SSD  growth 
  ...
  % mm in panels (a). Note, that the
    ...
    % FAG [The fluctuations are more evident in the time series for E_k. while 
    % it does increase initially in a2, in b2 there is little change. This is 
    % more relevant in addressing the criticism of the fluctuating growth rates
    % because intermittency in the flow may well explain the changes, but we 
    % adequately address the referee elsewhere, so I think we can omit this
    % comment and save space]
...
%FAG para
 \fg{For $\fg{\dot\sigma=0.2\SNr}$ up to 14\,Myr this peak is at
...
%FAG para
 ...
 due to SSD. 
 Tangling of the imposed field is \fg{initially} present, but is
 dominated by SSD. \fg{O}ur other models 
...
\acknowledgments
 \fg{We thank O. Gressel and D. Elstner for discussions inspiring this work,
